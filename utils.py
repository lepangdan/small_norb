...
class DCGAN(object):
    ...
    def build_model(self):
        ...
        self.accuracy = tf.reduce_mean(
            tf.cast(tf.equal(tf.argmax(self.D_logits, 1), tf.argmax(self.labels, 1)), tf.float32))
    ...
    def train(self, config):
        ...
        d_optim = tf.train.AdamOptimizer(config.learning_rate * 10, beta1=config.beta1).minimize(self.d_loss,
        ...
        for epoch in xrange(config.epoch):
            ...
            for idx in xrange(0, num_batches):
                ...
                _, summary_str = self.sess.run([d_optim, self.d_sum],
                                               feed_dict={self.images: batch_images, self.labels: batch_y,
                                                          self.z: batch_z})
                ...
                print("Epoch: [%2d] [%4d/%4d] time: %4.4f, d_loss: %.8f, g_loss: %.8f, accuracy: %.4f" % (
                epoch + 1, idx + 1, num_batches, time.time() - start_time, errD_fake + errD_real, errG, acc))

                if (idx + 1) % 100 == 0:
                    samples, d_loss, g_loss = self.sess.run([self.sampler, self.d_loss, self.g_loss],
                                                            feed_dict={self.z: sample_z,
                                                                       self.images: imgs[:self.sample_size, :, :, :],
                                                                       self.labels: targ[:self.sample_size, :]})
                    save_images(samples, [8, 8], './samples/train_{:02d}_{:04d}.png'.format(epoch, idx))
                    print("Epoch: %02d, Iter: %03d, d_loss: %.8f, Val accuracy: %.4f" % (
                    epoch + 1, idx + 1, d_loss, self.get_accuracy(val_x, val_y)))
        ...
        print
        'Test accuracy: %.4f' % (self.get_accuracy(img_t, label_t))
    ...
    def generator(self, z):
        ...
        self.h1, self.h1_w, self.h1_b = conv2d_transpose(h0, [self.batch_size, 8, 8, self.gf_dim * 4], name='g_h1',
                                                         with_w=True)
        h1 = tf.nn.relu(self.g_bn1(self.h1))

        h2, self.h2_w, self.h2_b = conv2d_transpose(h1, [self.batch_size, 16, 16, self.gf_dim * 2], name='g_h2',
                                                    with_w=True)
        h2 = tf.nn.relu(self.g_bn2(h2))

        h3, self.h3_w, self.h3_b = conv2d_transpose(h2, [self.batch_size, 32, 32, self.gf_dim * 1], name='g_h3',
                                                    with_w=True)
        h3 = tf.nn.relu(self.g_bn3(h3))

        h4, self.h4_w, self.h4_b = conv2d_transpose(h3, [self.batch_size, 96, 96, self.c_dim], d_h=3, d_w=3,
...
def get_training_data():
    fid_images = open('../datasets/smallnorb/smallnorb-5x46789x9x18x6x2x96x96-training-dat.mat', 'r', encoding='latin-1')
    fid_labels = open('../datasets/smallnorb/smallnorb-5x46789x9x18x6x2x96x96-training-cat.mat', 'r', encoding='latin-1')
    ...
    for idx in range(num_images):
        temp = fid_images.read(96 * 96)
        print('temp:',np.shape(np.fromstring(temp, 'uint8')))
        images[idx, :, :] = np.fromstring(temp, 'uint8').reshape(96, 96).T
